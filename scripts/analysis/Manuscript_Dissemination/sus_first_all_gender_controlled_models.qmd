---
title: "sus first all gender controlled models"
format: docx
date: today
editor: visual
execute:
...
# SUS First Linear Regression: All Gender

## Model 1: FA \~ SUS + Gender + Age
...
## Model 1: FA \~ SUS + Gender + Age + IQ

```{r}
models_fa_m1_gender <- roi_fa_allgender |>
  group_by(roi) |>
  group_map(
    ~ lm(FA_mean ~ sus + age + Gender, data = .x),
    .keep = TRUE
  )
...
results_fa_m1_gender_sus <- results_fa_m1_gender |>
  filter(term == "sus") |>
...
results_roi_fa_model2_sus_allgender <- results_roi_fa_model2_allgender |>
  filter(term == "sus") |>
  mutate(p_fdr = p.adjust(p.value, method = "fdr"))
...
#| execute:
#|   cache: false
results_roi_fa_model2_sus_allgender |>
  select(
    roi,
    estimate,
    std.error,
